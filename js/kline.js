...
DarkTheme.prototype.__construct = function () {
    ...
    this._fonts = [];
    this._fonts[Theme.Font.Default] = "10px Tahoma";
};
...
LightTheme.prototype.__construct = function () {
    ...
    if (KlineIns.reverseColor) {
        this._colors[Theme.Color.Positive] = "#13b5b1";
        this._colors[Theme.Color.Negative] = "#E70353";
    ...
    this._colors[Theme.Color.Grid1] = "#eee";
    this._colors[Theme.Color.Grid2] = "#ccc";
    this._colors[Theme.Color.Grid3] = "#bbb";
    this._colors[Theme.Color.Grid4] = "#aaa";
    this._colors[Theme.Color.TextPositive] = "#13b5b1";
    this._colors[Theme.Color.TextNegative] = "#E70353";
    ...
    this._colors[Theme.Color.LineColorNormal] = "#0000";
    ...
    this._fonts = [];
    this._fonts[Theme.Font.Default] = "10px Microsoft YaHei";
};
...
MainInfoPlotter.prototype.Draw = function (context) {
    ...
    if (lang == "zh-cn") {
        if (!Plotter.drawString(context, '    量:   ' + data.volume.toFixed(4), rect))
            return;
        if (!Plotter.drawString(context, '    振幅:   ' + amplitude.toFixed(4) + ' %', rect))
            return;
        
    } else if (lang == "en-us") {
        if (!Plotter.drawString(context, '   V:  ' + data.volume.toFixed(4), rect))
            return;

        if (!Plotter.drawString(context, '  AMPLITUDE:   ' + amplitude.toFixed(4) + ' %', rect))
            return;
    } else if (lang == "zh-tw") {
         if (!Plotter.drawString(context, '   量:  ' + data.volume.toFixed(4), rect))
            return;
...
DrawPriceLinesPlotter.prototype.draw = function (context) {
    ...
    Plotter.drawLine(context, this.startPoint.x, this.startPoint.y, this.areaPos.right, this.startPoint.y);
    context.fillText(text.toFixed(4), this.startPoint.x + 2, this.startPoint.y - 15);
};
...
$('body').on('click', '#sizeIcon', function () {
    ...
    if (KlineIns.isSized) {
        ...
        $("#sizeIcon").removeClass("chart_BoxSize");
        $("#sizeIcon").addClass("chart_BoxSize1");
        on_size();
        ...
        $("#sizeIcon").removeClass("chart_BoxSize1");
        $("#sizeIcon").addClass("chart_BoxSize");
...
function requestOverHttp() {
    $(document).ready(
        KlineIns.G_HTTP_REQUEST = $.ajax({
            ...
            created: Date.now(),
            beforeSend: function () {
               // KlineIns.intervalTime=KlineIns.range;
               var TimeStyle=proList[KlineIns.range];
               var jiayidui=buyAndSell.buyAndCoin+"/"+buyAndSell.market;
/*               if(KlineIns.range==60000&&refreshFlag>0){
                    TimeStyle="rt";
               }*/
                if(first){
                    this.url=this.url+"/"+TimeStyle+'/before/'+jiayidui; 
                }else{
                    this.url=this.url+"/"+TimeStyle+'/now/'+jiayidui;
                }  
            ...
            success: function (res) {
                ...
                if(first){
                        ...
                        $.each(item,function(k,v){
                            var smallArray=[];
                            smallArray.push(v.time);
                            smallArray.push(v.open);
                            smallArray.push(v.highest);
                            smallArray.push(v.lowest);
                            smallArray.push(v.close);
                            smallArray.push(v.volum);
                   ...
                   /* if(KlineIns.range==60000){
                            var smallArray=[];
                            smallArray.push(res.data.time);
                            smallArray.push(res.data.open);
                            smallArray.push(res.data.highest);
                            smallArray.push(res.data.lowest);
                            smallArray.push(res.data.close);
                            smallArray.push(res.data.volum);
                            newArray.push(smallArray);
                        
                    }else{*/
                            var smallArray=[];
                            smallArray.push(res.data.time);
                            smallArray.push(res.data.open-0);
                            smallArray.push(res.data.highest-0);
                            smallArray.push(res.data.lowest-0);
                            smallArray.push(res.data.close-0);
                            smallArray.push(res.data.volum-0);
                            newArray.push(smallArray);
                ...
                //res.data.depths={};
                resData.data["lines"]=newArray;
                // /res.data.kline=res.
                if (KlineIns.G_HTTP_REQUEST) {
                    requestSuccessHandler(resData);
                }

                //分时的时候，需要每秒刷新，60秒后重新加载
                if(KlineIns.range==60000){
                    refreshFlag++;
                }
                if(KlineIns.range==60000&&refreshFlag==60){
                    refreshFlag=0;
                    newArray=[];
                    first=true;
                }
...
function on_size(w, h) {
    ...
    toolBar.css({
        ...
        width: toolBarRect.w-2 + 'px',
        height: toolBarRect.h+2 + 'px'
    });
    if (toolPanelShown) {
        toolPanel.css({
    ...
    if (tabBarShown) {
        tabBar.css({
            ...
            top: tabBarRect.y + 'px',
            width: tabBarRect.w-2 + 'px',
            height: tabBarRect.h-2 + 'px'
        });
    }
