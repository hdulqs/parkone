...
Powerange.prototype.setRange = function(min, max) {
  if (typeof min === 'number' && typeof max === 'number' && !this.options.hideRange) {
    ...
    this.slider.querySelector('.range-handVal').innerHTML = '0%';
  }
};
...
Powerange.prototype.setValue = function (offset, size) {
  var part = percentage.from(parseFloat(offset), size)
	...
	if(this.options.decimal){
		var _max = ""+(this.options.max),
			_numPo = 1;
		if(_max.indexOf(".") != -1){
			//_numPo = _numPo + pingLing(_max.substring(_max.indexOf(".")+1).length);
			_numPo = _numPo + pingLing(this.options.decimalStep);
		}
		value = Math.floor(value * Number(_numPo)) / _numPo;
	}else{
		value = Math.floor(value);
	}
  changed = (this.element.value != value) ? true : false;

  this.slider.querySelector('.range-handVal').innerHTML = parseInt(Number(value/this.options.max)*100) >0? parseInt(Number(value/this.options.max)*100)+"%" : '';
...
Horizontal.prototype.setPosition = function(val) {
	var objC = this.slider.getAttribute("class");
	if(val > 0 && objC){
		this.slider.setAttribute("class",'range-bar actives');
	}else{
		this.slider.setAttribute("class",'range-bar');
	}
  this.handle.style.left = val + 'px';
  this.slider.querySelector('.range-quantity').style.width = val + 'px';
  var _lW = this.slider.querySelector('.range-handVal').innerHTML;
  _lW = Number(_lW.replace("%",""));
  //this.slider.querySelector('.range-handVal').style.left = (val-(_lW*0.15)) + 'px';
};
...
Vertical.prototype.setPosition = function(val) {
  ...
  //this.slider.querySelector('.range-handVal').style.left = (val>10? (val > 100) ? val-20 : val-10 : val) + 'px';
};
